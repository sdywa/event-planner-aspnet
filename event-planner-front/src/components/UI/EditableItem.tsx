...
export const EditableItem: FC<IEditableItemProps> = ({isActive, activeState, defaultState, open, close, remove, showButtons=true}) => {
    return (
        <div className="w-full">
            <div className={clsx(["flex justify-between items-center gap-2 w-full", isActive ? "flex" : "hidden"])}>
                {activeState}
                <div className="flex items-center gap-2">
                    <i className="fa-solid fa-check text-green text-xl cursor-pointer" onClick={() => close()}></i>
                </div>
            </div>
            <div className={clsx(["flex justify-between items-center gap-2 w-full", !isActive ? "flex" : "hidden"])}>
                {defaultState}
