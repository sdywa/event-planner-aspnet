...
export const Rating: FC<IEventRatingProps> = ({ event, reviewCallback }) => {
    const starCount = 5;
    const colorClass = "text-yellow";
    const starsRef = [...Array(starCount)].map(() =>
    ...
    const getOnStarClick = (index: number) => {
        return (e: React.MouseEvent<HTMLDivElement>) => {
            e.stopPropagation();
            e.preventDefault();
            if (!isClicked) reviewCallback(index + 1);
    ...
    const getOnStarHover = (index: number) => {
        return () => {
            ...
            for (let i = 0; i < starCount; i++) {
                ...
                else starsRef[i].current?.classList.add(colorClass);
            }
        };
    };
    ...
    return (
        <Link
            to={`/events/${event.id}`}
            className="mx-auto flex justify-center items-center gap-6 w-[48rem]"
        >
                ...
                <div className="flex gap-1 text-3xl text-lightgray">
                    {starsRef.map((ref, i) => (
                        <div
                            ...
                            onClick={getOnStarClick(i)}
                            onMouseEnter={getOnStarHover(i)}
