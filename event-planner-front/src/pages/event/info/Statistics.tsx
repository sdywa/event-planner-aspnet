        ...
        for (const ticket of statistics.tickets) {
            for (const item of data) {
                item[`ticket${ticket.id}`] =
                    item.name in ticket.sales ? ticket.sales[item.name] : 0;
            }
        }
        console.log(data);
        return data;
    ...
    useEffect(() => {
        ...
        if (eventId) {
            ...
            const getEvent = async () => {
                ...
                try {
                    const response = await EventService.getStatistics(
                        Number(eventId)
                    );
                    ...
                    <div className="flex flex-col gap-2 w-full">
                        <div className="flex gap-2 items-center">
                            ...
                            <Link
                                ...
                                className="hover:text-green transition-colors ease-in duration-150"
                            >
                        ...
                        {statistics && (
                            <Table
                                ...
                                data={statistics.tickets as never[]}
                                ceilsSchema={[
                                    (value: IEventTicketsStatistics) => (
                                        <div className="font-bold w-full overflow-hidden text-ellipsis whitespace-nowrap">
                                            {value.title}
                                        </div>
                                    ),
                                    (value: IEventTicketsStatistics) => (
