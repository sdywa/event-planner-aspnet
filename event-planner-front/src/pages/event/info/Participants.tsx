...
export const Participants: FC = () => {
    ...
    const [participants, setParticipants] =
        useState<IEventParticipantsResponse>();
    ...
    useEffect(() => {
        ...
        if (eventId) {
            ...
            const getEvent = async () => {
                ...
                try {
                    const response = await EventService.getParticipants(
                        Number(eventId)
                    );
        ...
        <PageLayout title={eventTitle}>
            <Modal active={modalActive} setActive={setActive}>
                <div className="flex justify-between items-center gap-4">
                    ...
                    <Link
                        to="#"
                        className="text-gray hover:text-darkgray transition-colors duration-150 ease-in align-middle"
                                    ...
                                    (value: IEventParticipant) => (
                                        <div className="w-6">
                                            <i
                                                ...
                                                onClick={async (e) => {
                                                    e.stopPropagation();
                                                    await EventService.deleteParticipant(
                                                        Number(eventId),
                                                        value.id
                                                    );
                                                    setParticipants(
                                                        (
                                                            await EventService.getParticipants(
                                                                Number(eventId)
                                                            )
                                                        ).data
                                                    );
