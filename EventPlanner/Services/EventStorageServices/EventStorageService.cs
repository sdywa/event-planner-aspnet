...
public class EventStorageService : CommonQueries<int, Event>, IEventStorageService
{
    ...
    {
        var entities = await IncludeValues().Where(e => e.CreatorId == creatorId).ToListAsync();
        return entities;
    }
    ...
    public async Task<Address> CreateAsync(Address entity) =>
        await _addressService.CreateAsync(entity);

    public async Task UpdateAsync(Address entity) => await _addressService.UpdateAsync(entity);

    public async Task<Question?> GetQuestionAsync(int id) => await _questionService.GetAsync(id);

    public async Task<List<Question>> GetQuestionsByEventAsync(int eventId) =>
        await _questionService.GetByEventAsync(eventId);
